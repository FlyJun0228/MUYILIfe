...
public class MainActivity extends FragmentActivity {
    ...
    private Class fragmentArray[] = {First.class, Second.class, Third.class, Fourth.class};              //定义数组来存放Fragment界面
    private int mImageViewArray[] = {R.drawable.housee, R.drawable.starr, R.drawable.ringg,              //定义数组来存放按钮图片
            R.drawable.vipp};
    private int mImageView[] = {R.drawable.btn_sure, R.drawable.btn_sure2, R.drawable.btn_sure3,
            R.drawable.btn_sure4};
    private String mTextviewArray[] = {"首页", "木易精品", "最新动态", "我的vip"};                     //Tab选项卡的文字
    ...
    public void initView() {
        ...
        mTabHost = (FragmentTabHost) findViewById(android.R.id.tabhost);                                        //实例化TanHost对象，得到Tabhost
        mTabHost.setup(this, getSupportFragmentManager(), R.id.realtabcontent);
        int count = fragmentArray.length;                                                                      //得到fragment的个数
        for (int i = 0; i < count; i++) {                                                                         //为每一个Tab按钮设置图标、文字和内容
            TabHost.TabSpec tabSpec = mTabHost.newTabSpec(mTextviewArray[i]).setIndicator(getTabItemView(i));
            mTabHost.addTab(tabSpec, fragmentArray[i], null);

            mTabHost.setOnTabChangedListener(new TabHost.OnTabChangeListener() {                                  //将Tab按钮添加进Tab选项卡中

                @Override
                public void onTabChanged(String tabId) {                                                          //设置Tab按钮的背景
